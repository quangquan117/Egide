...
export async function fill_shop(type){
    ...
    for (let i = result["admin"] == true ? 0 : 1 ; i < all_li.length; i++) {
        ...
        button_achat.addEventListener("click", () => {
            if (type == "type_soldat")
                var id = all_li[i].ID_Soldat;
            else if (type == "batiment")
                var id = all_li[i].ID_Batiment;
        ...
        if (result && result["admin"] == true) {
            ...
            button_supp.addEventListener("click", () => {
                ...
                delete_data(type, id);
                if (type == "type_soldat")
                    document.location.href = "http://localhost/projet_final/store.php?type=soldat";
                else if (type == "batiment")
                    document.location.href = "http://localhost/projet_final/store.php?type=batiment";
            });
